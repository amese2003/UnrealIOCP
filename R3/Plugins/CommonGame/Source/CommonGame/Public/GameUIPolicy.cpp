...
{
	// UCommonUserWidget에 SlateWidget을 가져와서 Parent와 연결고리를 끊어 놓는다
	TWeakPtr<SWidget> LayoutSlageWidget = Layout->GetCachedWidget();

	if (LayoutSlageWidget.IsValid())
	{
...
void UGameUIPolicy::CreateLayoutWidget(UCommonLocalPlayer* LocalPlayer)
{
	...
	{
		// LayoutWidgetClass가 있을 경우 (또! UPrimaryGameLayout은  Abstract이고 이를 상속받는 Class이여야 한다)
...
{
	// PlayerController가 업데이트 되면 GameLayout을 업데이트해주기 위해 Delegate를 추가한다
...
void UGameUIPolicy::NotifyPlayerDestroyed(UCommonLocalPlayer* LocalPlayer)
{
	...
	// Player가 Destory되므로, OnPlayerControllerSet에서 제거하자
