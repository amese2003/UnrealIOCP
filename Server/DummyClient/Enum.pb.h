...
enum CreatueType : int {
  CREATURE_TYPE_NONE = 0,
  CREATURE_TYPE_PLAYER = 1,
  CREATURE_TYPE_MONSTER = 2,
  CREATURE_TYPE_NPC = 3,
  CreatueType_INT_MIN_SENTINEL_DO_NOT_USE_ = std::numeric_limits<int32_t>::min(),
  CreatueType_INT_MAX_SENTINEL_DO_NOT_USE_ = std::numeric_limits<int32_t>::max()
};
bool CreatueType_IsValid(int value);
constexpr CreatueType CreatueType_MIN = CREATURE_TYPE_NONE;
constexpr CreatueType CreatueType_MAX = CREATURE_TYPE_NPC;
constexpr int CreatueType_ARRAYSIZE = CreatueType_MAX + 1;

const ::PROTOBUF_NAMESPACE_ID::EnumDescriptor* CreatueType_descriptor();
template<typename T>
inline const std::string& CreatueType_Name(T enum_t_value) {
  static_assert(::std::is_same<T, CreatueType>::value ||
    ::std::is_integral<T>::value,
    "Incorrect type passed to function CreatueType_Name.");
  return ::PROTOBUF_NAMESPACE_ID::internal::NameOfEnum(
    CreatueType_descriptor(), enum_t_value);
}
inline bool CreatueType_Parse(
    ::PROTOBUF_NAMESPACE_ID::ConstStringParam name, CreatueType* value) {
...
template <> struct is_proto_enum< ::Protocol::CreatueType> : ::std::true_type {};
template <>
